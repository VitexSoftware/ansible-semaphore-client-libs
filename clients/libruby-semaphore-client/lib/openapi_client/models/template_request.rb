  ...
  class TemplateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TemplateRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TemplateRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@repository_id.nil? && @repository_id < 1
        invalid_properties.push('invalid value for "repository_id", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@repository_id.nil? && @repository_id < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] repository_id Value to be assigned
    def repository_id=(repository_id)
      if !repository_id.nil? && repository_id < 1
        fail ArgumentError, 'invalid value for "repository_id", must be greater than or equal to 1.'
      end
    ...
    def hash
      [project_id, inventory_id, repository_id, environment_id, view_id, _alias, playbook, arguments, description, override_args].hash
    end
